...
# Kullanıcı mesajını hedef bot yanıtı ile eşleştirmek için dict
pending_messages = {}  # user_id -> mesaj bekleniyor (True/False)
...
async def send_to_user(user_id, file=None, text=None):
    try:
    ...
    try:
        if file:
            await userbot.send_file(entity=entity, file=file, caption=text, force_document=True)
        elif text:
...
async def target_bot_handler(event):
    """
    Hedef bottan gelen yanıtları doğru kullanıcıya iletir
    """
    # Yanıt bekleyen kullanıcıları al
    for user_id, waiting in pending_messages.items():
        if waiting:
            pending_messages[user_id] = False  # yanıt verildi
            ...
            try:
                ...
                if event.message.file:
                    ...
                    await send_to_user(user_id, file=filepath, text=event.raw_text)
                    os.remove(filepath)
            ...
            break  # sadece ilk bekleyen kullanıcıya gönder
...
async def user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Kullanıcı mesaj attı, yanıt bekleniyor
