...
async def target_bot_handler(event):
    ...
    user_id = list(active_requests.keys())[0]

    try:

        # DOSYA VARSA
        if event.document or event.file or event.media:
...
async def user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await request_queue.put((user_id, text))

    await update.message.reply_text("🔎 Sorgu gönderildi...")
...
async def request_worker():
    ...
    while True:
        ...
        user_id, text = await request_queue.get()
