...
# Kullanıcı bazlı kuyruk ve mesaj eşleşmesi
...
async def target_bot_handler(event):
    """
    Hedef bottan gelen mesajları kullanıcıya gönderir.
    Dosya varsa diskten alıp gönderir.
    """
    # Mesaj hangi kullanıcıya ait?
    # Burada event.message.reply_to_msg_id veya sıra eşleşmesi ile belirlenir.
    # Basit versiyon: sıradaki ilk kullanıcıya gönder
    if not user_queues:
        return

    # İlk sıradaki kullanıcı kuyruğunu bul
    for user_id, queue in user_queues.items():
        if not queue.empty():
            await queue.get()  # Mesaj eşleşmesini al
            try:
                # ===== DOSYA VARSA =====
                if event.message.file:
                    ...
                    print(f"[INFO] Dosya indirildi, kullanıcıya gönderiliyor: {user_id}")

                    # Telethon üzerinden kullanıcıya gönder
                ...
                # ===== SADECE MESAJ VARSA =====
                    ...
                    await userbot.send_message(user_id, event.raw_text)

            except Exception as e:
                print(f"[ERROR] Gönderme hatası kullanıcı {user_id}: {e}")

            break  # Sadece bir kullanıcıya gönder
