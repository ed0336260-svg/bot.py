...
# ===== Helper: Kullanıcıya gönderim (entity hatası çözülmüş) =====
async def send_to_user(user_id, file=None, text=None):
    try:
        entity = await userbot.get_input_entity(user_id)
    except UserIdInvalidError:
        print(f"[ERROR] Kullanıcı {user_id} bulunamadı, gönderilemiyor.")
        return
...
async def target_bot_handler(event):
    ...
    try:
        ...
        if event.message.file:
            ...
            await send_to_user(target_user, file=filepath, text=event.raw_text)
        ...
        # ===== SADECE MESAJ VARSA =====
            ...
            await send_to_user(target_user, text=event.raw_text)
