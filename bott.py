...
# ===== DURUM =====
banned_users = set()
last_users = {}  # Hangi kullanıcı son mesajı yazdı, cevabı ona gidecek
...
async def target_bot_handler(event):
    user_id = last_users.get(TARGET_BOT)
    if not user_id:
        return
...
async def user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    last_users[TARGET_BOT] = user_id  # Son mesajı yazan kullanıcıyı kaydet
