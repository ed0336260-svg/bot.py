...
# ===== DURUM YÖNETİMİ =====
banned_users = set()
pending_users = defaultdict(list)  # {user_id: [text veya {'file': buffer, 'filename': filename, 'caption': ...}]}
        ...
        try:
            if isinstance(item, dict) and 'file' in item:
                await bot_app.bot.send_document(
                    chat_id=user_id,
                    document=InputFile(item['file'], filename=item['filename']),
...
async def target_bot_handler(event):
    ...
    for user_id in list(pending_users.keys()):
        ...
        if event.file:
            ...
            queue[0] = {'file': buffer, 'filename': filename, 'caption': event.raw_text or ""}
        else:
