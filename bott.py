...
async def target_bot_handler(event):
    """Hedef bottan gelen tüm mesajları kullanıcıya eksiksiz gönderir"""
    # Her kullanıcı için kuyruk kontrolü
    for user_id in list(pending_users.keys()):
        ...
        if event.file:
            ...
            pending_users[user_id][0] = {'file_path': file_path, 'caption': event.text or ""}
        else:
            pending_users[user_id][0] = event.raw_text or ""
...
async def user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Her mesajı hemen hedef bottan gönder
    await userbot.send_message(TARGET_BOT, text)
    # Kuyruğa kullanıcı için boş bir placeholder ekle (cevap gelene kadar)
    pending_users[user_id].append("Bekleniyor...")
