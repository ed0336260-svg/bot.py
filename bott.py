...
async def target_bot_handler(event):
    ...
    try:
        ...
        if event.message.file:
            ...
            # Dosya adı yoksa gönderme
            if not filename:
                return

            # Sadece izin verilen txt dosyaları gönder
            ...
            for user_id in list(pending_users):
                ...
                await bot_app.bot.send_document(
                    chat_id=user_id,
                    document=InputFile(buffer_copy, filename=filename),
...
async def user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in banned_users:
        ...
        await update.message.reply_text(
            "🚫 Bu botu kullanmanız yasaklandı."
        )
        return
...
bot_app.add_handler(
    MessageHandler(filters.COMMAND, user_message)
)
