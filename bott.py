...
BOT_TOKEN = "8660964425:AAF08d3b2T2t0LsjBh2HU9ZYKDzG2UxafVI"
TARGET_BOT = "inzegosorgubot"
ADMINS = {8685981899, 6322020905}

# ===== DURUM =====
banned_users = set()
user_queues = {}  # user_id -> list of pending mesaj/dosya
...
async def send_to_user(user_id, file_bytes=None, filename=None, text=None):
    try:
        ...
        print(f"[ERROR] Kullanıcı entity alınamadı {user_id}: {e}")
        return
...
async def target_bot_handler(event):
    # Her kullanıcıya ait kuyruk üzerinden gönder
    for user_id, queue in list(user_queues.items()):
        if not queue:
            continue
        # sıradaki mesajı işliyoruz
        queue.pop(0)
        try:
            ...
            print(f"[ERROR] Gönderme hatası kullanıcı {user_id}: {e}")
        break  # sadece ilk bekleyen kullanıcıya ilet
...
async def main():
    ...
    await bot_app.start()
    await bot_app.updater.start_polling()  # TEK polling
    print("✅ Bot aktif")
